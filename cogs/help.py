...
class help(commands.Cog, name="Help"):
    """Shows this message. To get info regarding any command or cog"""
    ...
    async def help(self, ctx, cmd: str = None):
        ...
        if not cmd:
            ...
            for cog in ['Moderator', 'Nuke', 'Kill']:

                if (self.bot.cogs[cog].description is
                        None) or (self.bot.cogs[cog].description == ""):
                    description = "N/A"
                else:
                    description = self.bot.cogs[cog].description

                e = discord.Embed(title=f"Help with {cog}",
                                  description=f"```\n{description}```",
                                  timestamp=datetime.utcnow(),
                                  color=discord.Color.random())
                temp.append(e)
        ...
        if cmd:
            ...
            if cog in ['Moderator', 'Nuke', 'Kill']:

                if not self.bot.cogs[cog].description:
                    description = "No description"
                else:
                    description = self.bot.cogs[cog].description

                embed = discord.Embed(title=f"Help with {cog}",
                                      description=f"```\n{description}```",
                                      timestamp=datetime.utcnow(),
                                      color=discord.Color.random())
                commands = []
                ...
                for command in self.bot.cogs[cog].get_commands():
                    ...
                    cmd_em.add_field(
                        name="Usage",
                        value=f"```\n[p]{command.name} {command.signature}```",
                        inline=True)
